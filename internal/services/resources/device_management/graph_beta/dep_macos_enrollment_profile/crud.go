...
// Create handles the Create operation for DEP macOS Enrollment Profile resources.
func (r *DepMacOSEnrollmentProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	depId, ok := r.getOrResolveDepId(ctx, object.DepOnboardingSettingsId, &resp.Diagnostics)
	if !ok {
		return
	}
	...
	if created == nil || created.GetId() == nil {
		resp.Diagnostics.AddError(
			...
			fmt.Sprintf("API returned a %s with no ID", ResourceName),
		)
		return
	}
...
func (r *DepMacOSEnrollmentProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	MapRemoteStateToTerraform(ctx, &object, remoteProfile, depId, object.AdminAccountPassword)
...
// Update handles the Update operation for DEP macOS Enrollment Profile resources.
func (r *DepMacOSEnrollmentProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var plan DepMacOSEnrollmentProfileResourceModel
	var state DepMacOSEnrollmentProfileResourceModel
	...
	// Prefer plan's dep_onboarding_settings_id (user may be changing it), fallback to state
	providedId := plan.DepOnboardingSettingsId
	if providedId.IsNull() || providedId.ValueString() == "" {
		providedId = state.DepOnboardingSettingsId
	}
	depId, ok := r.getOrResolveDepId(ctx, providedId, &resp.Diagnostics)
	if !ok {
		return
	}
	...
	// Sync plan values into state before Read so ReadWithRetry preserves them correctly.
...
// Delete handles the Delete operation for DEP macOS Enrollment Profile resources.
func (r *DepMacOSEnrollmentProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var object DepMacOSEnrollmentProfileResourceModel
